...
class SelectionCard extends React.Component {
    ...
    renderButton(algorithm) {
        ...
        if (algorithm.name === this.state.selectedAlgorithm.name) {
            return (
                <Button variant='outlined' id={algorithm.name} className="btn selected" onClick={()=>{this.setSelectedAlgorithm(algorithm)}}>{algorithm.name} {this.renderIcon(algorithm, true)}</Button>
            )
        } else {
            return (
                <Button className="btn" id={algorithm.name} onClick={()=>{this.setSelectedAlgorithm(algorithm)}}>{algorithm.name} {this.renderIcon(algorithm, false)}</Button>
            )
        }
        
    }

    renderIcon(algorithm, state) {
        if (localStorage.getItem(algorithm.path) == 'true' ? true : false) {
            if(state){
                return (
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="var(--cardColor)" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
